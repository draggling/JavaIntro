...
public class CRUDBook {
	...
	public void addBook() throws ClassNotFoundException, SQLException {
		...
		try {
			...
			try {
				bdao.addBook(temp);
				Integer newBookId = bdao.getBooIdkWithTitle(title);
				book = new Book(newBookId, title, publisherId);
				...
				case(3):
					...
					case(1):
						...
						try {
							...
							*/
							List<Author> remainingAuthors = adao.readAllAuthors();
								...
								try {
									...
									if(authorOption < 1 || authorOption >= count) {
										...
										try {
											...
											*/
											adao.deleteBookAuthorPair(book.getBookId(), book.getAuthors().get(authorOption-1).getAuthorId());
				...
				case(4):
					...
					case(1):
						...
						try {
							...
							List<Genre> remainingGenres = gdao.readAllGenres();
								...
								try {
									...
									if(genreOption < 1 || genreOption >= count) {
										...
										try {
											...
											*/
											gdao.deleteBookGenrePair(book.getBookId(), book.getGenres().get(genreOption-1).getGenreId());
